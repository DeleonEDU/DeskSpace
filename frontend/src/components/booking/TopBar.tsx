  ...
  return (
    <header className="flex items-center justify-between rounded-full bg-card/90 px-4 py-3 ring-1 ring-border/60 shadow-[var(--shadow-soft)] backdrop-blur-xl">
      <Link to="/" className="flex items-center gap-3 pl-2">
        ...
        <span className="font-display text-lg font-semibold tracking-tight text-foreground">DeskSpace</span>
      </Link>
        ...
        {user ? (
          <>
            ...
            >
              ...
              <span className="hidden text-sm font-medium sm:block text-foreground">{user.first_name} {user.last_name}</span>
            </Link>
            
            <div className="h-5 w-px bg-border/80" />

            <Button variant="ghost" size="icon" onClick={logout} className="rounded-full text-muted-foreground hover:text-destructive hover:bg-destructive/10 transition-colors" title="Вийти">
          ...
          <div className="flex items-center gap-2">
            <Link to="/login">
              <Button variant="ghost" className="rounded-full px-5">Увійти</Button>
            </Link>
            <Link to="/register">
